...
pub trait MessageSender {
    async fn reply_to_thread<T: SlackMessageTemplate + Send + Sync>(&self,
        message: T,
...
impl MessageSender for SlackHyperClient {
    async fn reply_to_thread<T: SlackMessageTemplate + Send + Sync>(&self, message: T, event: &SlackMessageEvent) -> Result<(), Box<dyn Error + Send + Sync>> {
