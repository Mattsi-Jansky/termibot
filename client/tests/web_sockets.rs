...
use slack_morphism::{prelude, slack_block_item, slack_blocks};
...
fn start_websocket_server() -> WebSocketStream<TcpStream> {
...
async fn should_initiate_socket_mode_connection() {
    let websocket_server = async move {
        let mut stream = start_websocket_server();
        stream
