...
impl Plugin for EmojiChangelogPlugin {
    async fn on_event(&self, event: &Event, dependencies: &Dependencies) -> Vec<Action> {
        match event {
            Event::EmojiChanged(emoji_event) => {
                match emoji_event {
                    ...
                    EmojiChangedEvent::Remove(_) => {vec![]}
                    EmojiChangedEvent::Rename(_) => {vec![]}
                }
            },
            _ => {vec![]}
        }
...
mod tests {
    ...
    async fn given_emoji_add_event_send_message_to_notification_channel() {
        ...
        let event = Event::EmojiChanged(EmojiChangedEvent::Add(AddEmojiEvent { id: MessageId::from("testid"), name: "newmoji".to_string() }));

        let mut result = plugin.on_event(&event, &dependencies).await;

        assert_eq!(1, result.len());
        assert_eq!(Action::MessageChannel {
